...
`update-releases.mjs` in a scratch folder against the same fake API. `.github/workflows/tests.yml` runs them on every
...
| `scripts/` | `serve.mjs` (preview), `test-site.mjs` (tests), `update-releases.mjs` (snapshot) |
...
| `.github/workflows/` | The hourly snapshot refresh (`refresh-releases.yml`) and the tests (`tests.yml`) |
